...
#include "ui_zKONAcontroldialog.h"
...
std::list<std::string> ZKONAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZKONAControlDialog::listMints;

ZKONAControlDialog::ZKONAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZKONAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZKONAControlDialog::~ZKONAControlDialog()
{
    delete ui;
}

void ZKONAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKONAControlDialog::updateList()
{
...
void ZKONAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKONAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZKONA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZKONAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZKONAControlDialog::GetSelectedMints()
{
...
void ZKONAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
