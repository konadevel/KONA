                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zKONA ought to be enough for anybody." - Bill Gates, 2017
    ui->zKONApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzKONASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzKONA->setEnabled(false);
        ui->pushButtonMintzKONA->setToolTip(tr("zKONA is currently disabled due to maintenance."));

        ui->pushButtonSpendzKONA->setEnabled(false);
        ui->pushButtonSpendzKONA->setToolTip(tr("zKONA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zKONApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzKONA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKONA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zKONA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zKONA in ") + 
...
void PrivacyDialog::on_pushButtonSpendzKONA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKONA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zKONA
        sendzKONA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zKONA
    sendzKONA();
}

void PrivacyDialog::on_pushButtonZKONAControl_clicked()
{
    ZKONAControlDialog* zKONAControl = new ZKONAControlDialog(this);
    zKONAControl->setModel(walletModel);
    zKONAControl->exec();
}

void PrivacyDialog::setZKONAControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzKONASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzKONA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid KONA Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zKONApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zKONA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " KONA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zKONA</b>";
    ...
    // use mints from zKONA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZKONAControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZKONAControlDialog::GetSelectedMints();
    }
    ...
    // Clear zKONA selector in case it was used
    ZKONAControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zKONA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " KONA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zKONA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zKONA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zKONA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zKONA "));
    ui->labelzKONAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzKONASyncStatus->setVisible(fShow);
}
